...
工作流后端已经完成领域模型、动作路由与人工审核闭环：支持定义与节点配置、发布校验、实例启动、任务完成/失败/重试、审核通过/驳回/退回、候选用户或角色筛选、操作幂等和完整执行时间线。数据库脚本位于 `backend/db/workflow.sql`，领域说明位于 `docs/workflow-domain-design.md`。
