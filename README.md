...
AI 研发流水线已经进入资料与需求阶段：研发项目工作台支持上传需求资料，文本、Markdown、Word 和 Excel 可直接抽取内容，PDF、图片和演示文稿会保留原文件并等待 AI 解析器；已解析资料可以生成按模块组织的功能点草稿，用户能够编辑、通过或驳回，全部通过后项目推进到原型准备阶段。数据库脚本位于 `backend/db/workflow.sql`，领域说明位于 `docs/workflow-domain-design.md`。

当前真实回归路径为：`生成 -> 审核驳回 -> 返工 -> 再次审核通过 -> 交付`。本地回归脚本从 `PIG_ADMIN_PASSWORD` 环境变量或 `-Password` 参数读取管理员密码，不在仓库中保存默认口令。

项目资料与功能点回归脚本为 `scripts/smoke-project-intake.ps1`，验证 `创建项目 -> 上传资料 -> 需求分析 -> 功能点审核 -> 原型准备`。
